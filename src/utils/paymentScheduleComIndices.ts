...
export const generatePaymentScheduleComIndices = (
  ...
  console.log('=== INICIANDO CRONOGRAMA COM ÍNDICES REALISTAS ===');
  ...
  // Parcelas mensais com cálculos usando índices realistas
  for (let i = 1; i <= data.installmentsCount; i++) {
    // 1. Aplicar correção usando índice específico do mês
    const correctedBalance = calcularSaldoComIndice(
      balance,
    ...
    if (i < data.installmentsCount) {
      ...
      const currentPropertyValue = calcularValorCorrigido(
        data.propertyValue,
        configIndices.correcaoMonetaria.tipo,
  ...
  const finalPropertyValue = calcularValorCorrigido(
    data.propertyValue,
    configIndices.correcaoMonetaria.tipo,
    configIndices.valorizacao.tipo,
  ...
  console.log('=== RESUMO CRONOGRAMA COM ÍNDICES ===');
